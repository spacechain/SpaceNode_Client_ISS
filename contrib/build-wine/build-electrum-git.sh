...
echo "Done.=======================================00000000000000000001"

# Install frozen dependencies
$PYTHON -m pip install --no-warn-script-location -r "$CONTRIB"/deterministic-build/requirements.txt
echo "Done.=======================================00000000000000000002"

$PYTHON -m pip install --no-warn-script-location -r "$CONTRIB"/deterministic-build/requirements-hw.txt
echo "Done.=======================================00000000000000000003"

pushd $WINEPREFIX/drive_c/electrum
echo "Done.=======================================00000000000000000004"

# see https://github.com/pypa/pip/issues/2195 -- pip makes a copy of the entire directory
info "Pip installing Electrum. This might take a long time if the project folder is large."
echo "Done.=======================================00000000000000000005"
#$PYTHON -m pip install --user  --upgrade pip
#$PYTHON -m pip install --no-warn-script-location .
echo "Done.=======================================00000000000000000006"

popd
echo "Done.=======================================00000000000000000007"


rm -rf dist/
echo "Done.=======================================00000000000000000008"

# build standalone and portable versions
info "Running pyinstaller..."
echo "Done.=======================================00000000000000000009"

wine "$PYHOME/scripts/pyinstaller.exe" --noconfirm --ascii --clean --name $NAME_ROOT-$VERSION -w deterministic.spec
echo "Done.=======================================000000000000000000022"


# set timestamps in dist, in order to make the installer reproducible
pushd dist
echo "Done.=======================================000000000000000000023"

find -exec touch -d '2000-11-11T11:11:11+00:00' {} +
echo "Done.=======================================000000000000000000024"

popd
echo "Done.=======================================000000000000000000025"

info "building NSIS installer"
echo "Done.=======================================000000000000000000026"

# $VERSION could be passed to the electrum.nsi script, but this would require some rewriting in the script itself.
wine "$WINEPREFIX/drive_c/Program Files (x86)/NSIS/makensis.exe" /DPRODUCT_VERSION=$VERSION electrum.nsi
echo "Done.=======================================000000000000000000027"


cd dist
echo "Done.=======================================000000000000000000028"

mv electrum-setup.exe $NAME_ROOT-$VERSION-setup.exe
echo "Done.=======================================000000000000000000029"

cd ..

info "Padding binaries to 8-byte boundaries, and fixing COFF image checksum in PE header"
echo "Done.=======================================000000000000000000033"
